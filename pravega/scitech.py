...
@blueprint.route("/ideathon/register", methods=("GET","POST"))
def register_for_ideathon():
    event_name = "ideathon"
    amount = 10000
    if request.method == "GET":
        return render_template(f"scitech/registration/registration_{event_name}.html");
    if request.method == "POST":
        details = {
                    "participant_name" : request.form["participant_name"],
                    "participant_institution" : request.form['school'],
                    "participant_email" : request.form["email"],
                    "participant_phone" : request.form['mobile'],
        ...
        if existing is None:
            ...
            msg['From'] = "Pravega IISc"
            msg['To'] = toaddr
            msg['Subject'] = "Registration for Ideathon by Pravega IISc"
            body = f"""Hello Enthusiast,
Thank you for registering for Ideathon! We hope you have fun!
...
Registered details are:
Name:   {details['participant_name']}
Institution: {details['participant_institution']}
Email:  {details['participant_email']}
Phone:  {details['participant_phone']}
Offline Prelims Registration: {"Yes" if details['participant_offline_reg'] else "No"}
Online Prelims Registration: {"Yes" if details['participant_online_reg'] else "No"}

THIS MAIL IS AUTOGENERATED. DO NOT REPLY TO THIS MAIL.
For any queries, mail ideathon.pravega@gmail.com"""
            msg.attach(MIMEText(body, 'plain'))
